...
interface SparkPostAttachment {
  name: string;
  type: string;
  data: string | ReadableStream; // Base64 encoded content or stream (before conversion)
}
...
function createSampleAttachments() {
  console.log('📎 Sample attachment files are already available in examples/ directory\n');
}
...
Generated by json-streamify example`,
  ...
  const jsonStream = jsonStreamify(transmission);
  ...
  transmission.content.attachments?.forEach((attachment, index) => {
    ...
    console.log(
      `      Source: ${attachment.data instanceof ReadableStream ? 'ReadableStream' : 'Unknown'}`
    );
  });
  ...
  try {
    const result = await wretch(SPARKPOST_API_URL)
      ...
      .options({ duplex: 'half' }) // Enable duplex streaming
      .body(jsonStream)
      .post()
      .json<{
        results: {
...
async function main() {
  try {
    ...
    console.log('\n✅ Demo completed - sample files remain in examples/ directory');
  } catch (error) {
